...
```bash
openclaw-cn plugins install @clawdbot/nextcloud-talk
```

Local checkout (when running from a git repo):
```bash
openclaw-cn plugins install ./extensions/nextcloud-talk
```
...
- Approve via:
  - `openclaw-cn pairing list nextcloud-talk`
  - `openclaw-cn pairing approve nextcloud-talk <CODE>`
